...
const Profile = () => {
  ...
  const [avatarUrl, setAvatarUrl] = useState<string | StaticImageData>(FirstImag)
  const { user } = useStore()
        ...
        <div className="mb-5">
          <div className="flex flex-col text-sm items-center justify-center mb-5">
            <div className="relative w-24 h-24 mb-5">
              <Image src={avatarUrl} className="rounded-full object-cover" alt="avatar" fill  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw"/>
            </div>
            <input type="file" id="avatar" title="アバター画像を選択" onChange={onUploadImage} />
